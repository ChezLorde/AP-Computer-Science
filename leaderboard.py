...
# Used to create a turtle leaderboard
...
def get_scores(file_name):
  ...
  for line in leaderboard_file:
    ...
    # (DONE) 3: use a while loop to index beyond the comma, skipping the player's name
    while (line[index] != ","):
      index += 1
    index += 1

    # (DONE) 4: use a while loop to get the score
    while (line[index] != "\n"):
      leader_score = leader_score + line[index]
      index += 1

    # (DONE) 5: add the player score to the scores list
...
def update_leaderboard(file_name, leader_names, leader_scores,  player_name, player_score):
  ...
  # (DONE) 8: loop through all the scores in the existing leaderboard list
  for index in range(len(leader_scores)):
    # (DONE) 9: check if this is the position to insert new score at
    if (player_score >= leader_scores[index]):
      break
    #else:
      #index = index + 1
  
  # (DONE) 10: insert new player and score
  leader_scores.insert(index, player_score)
  leader_names.insert(index, player_name)

  # (DONE) 11: keep both lists at 5 elements only (top 5 players)
  while len(leader_scores) > 5:
    leader_scores.pop()
    leader_names.pop()
  
  # (DONE) 12: store the latest leaderboard back in the file
  leaderboard_file = open(file_name, "w")  # this mode opens the file and erases its contents for a fresh start

 
  # (DONE) 13 loop through all the leaderboard elements and write them to the the file
  index = 0
...
def draw_leaderboard(high_scorer, leader_names, leader_scores, turtle_object, player_score):
  ...
  # (DONE) 14: display message about player making/not making leaderboard
  if high_scorer:
  ...
  # (DONE) 15: Display a gold/silver/bronze message if player earned a gold/silver/or bronze medal; display nothing if no medal
